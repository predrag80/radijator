...
';
		...
		public function show_header( $show = true ) {
			...
			$output .= '<div class="flippercode-ui">
                    ...
                    <div class="fc-7 col-sm-6 col-xs-6 product-annoucement">' . $plugin_updates['annoucement'] . '</div>
		...
		public static function get_element_html( $name, $type, $atts ) {
			...
			if ( 'hidden' == $type ) {
				...
				if ( isset( $atts['required'] ) && ! empty( $atts['required'] ) ) {

					$element_output .= apply_filters( 'wpgmp_input_label_' . $name, '<div class="fc-3"><label for="' . $name . '">' . $atts['label'] . '</label>' . self::element_mandatory( $atts['required'] ) . '</div>' );
				} else if ( $atts['label'] != '' ) {

					$element_output .= apply_filters( 'wpgmp_input_label_' . $name, '<div class="fc-3"><label for="' . $name . '">' . $atts['label'] . '</label>' . '</div>' );
				}

				$element_output .= ( isset( $atts['before'] ) && $atts['before'] != '' ) ? $atts['before'] : '<div class="fc-8">';
				$element_output .= call_user_func( 'FlipperCode_HTML_Markup::field_' . $type, $name, $atts );
				$element_output .= ( isset( $atts['after'] ) && $atts['after'] != '' ) ? $atts['after'] : '</div>';
				return $element_output;
			}
		...
		public static function get_all_templates( $name, $atts ) {
			...
			<div class='fc-controls'>
               <div class='fc-accordion'>
                        <dl>
                            <dt class='fc-accordion-tab active'><a href='javascript:void(0)'>Font </a></dt>
                            <dd class='fc-acc-child active'>
				...
				if ( ! empty( $fonts ) ) {
					...
					foreach ( $fonts as $k => $v ) {
							...
							<dt class='fc-accordion-tab'><a href='javascript:void(0)'>Margin & Padding </a></dt>
                            <dd class='fc-acc-child'>
				            ...
				            <dt class='fc-accordion-tab'><a href='javascript:void(0)'>Background </a></dt>
                            <dd class='fc-acc-child'>
